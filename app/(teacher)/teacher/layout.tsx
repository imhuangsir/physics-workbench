...
import { LogOut, Users, BookOpen, ClipboardList, LayoutDashboard, ScanText, Megaphone } from "lucide-react";
...
const NAV = [
  ...
  { href: "/teacher/import", label: "导入", icon: ScanText },
  { href: "/teacher/assignments", label: "作业", icon: ClipboardList },
  { href: "/teacher/announcements", label: "公告", icon: Megaphone },
];
        ...
        <nav className="flex gap-1 overflow-x-auto border-t px-2 py-1 md:hidden">
          {NAV.map((n) => (
            <Link key={n.href} href={n.href}
              className={cn("shrink-0 rounded-full px-3 py-1.5 text-sm", isActive(n.href) ? "soft-violet" : "text-muted-foreground")}>
              {n.label}
            </Link>
          ))}
        </nav>
        ...
        <aside className="sticky top-14 hidden h-[calc(100dvh-3.5rem)] w-52 shrink-0 border-r p-3 md:block">
          <nav className="space-y-1">
            {NAV.map((n) => {
              ...
              return (
                <Link key={n.href} href={n.href}
                  className={cn("flex items-center gap-3 rounded-xl px-3 py-2 text-sm", isActive(n.href) ? "soft-violet font-medium" : "text-muted-foreground hover:bg-secondary")}>
